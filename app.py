...
SOLUTION_SYSTEM_PROMPT = (
    ...
    "用加粗关键词引导每个设计要点，每个要点仅 1-2 句话，精炼不冗余。参考格式：\n"
    "**控制平面:** 位于 East US 2 的 admin 系列资源组，作为租户的独立管理账户。\n"
    "**数据平面:** 位于 East US 的 test 系列资源组，承担全部模型推理算力。\n"
    "**严禁**对每个要点展开为多个子句或罗列子功能，模仿参考模板的精炼风格。\n\n"
    "## 六、安全架构\n"
    "每个要点用加粗关键词引导，仅 1 句话描述，不展开。例如：\n"
    "**数据沙箱:** 利用 AI Foundry 项目隔离机制，确保各租户数据在物理存储上完全隔离。\n"
    "**托管标识:** 所有资源间调用通过 Azure AD 托管标识认证，杜绝 API Key 泄露。\n\n"
    "## 七、集成架构\n"
    "每个要点用加粗关键词引导，仅 1 句话描述，不展开。例如：\n"
    "**SDK 封装:** 向客户提供封装好的 SDK，传入 Tenant-ID 即可路由至对应资源池。\n"
...
INFRA_SYSTEM_PROMPT = (
    ...
    "## 五、解决方案设计\n"
    "概述部署策略（区域选择、网络架构等）。用加粗关键词引导每个设计要点，每个要点仅 1 句话，精炼不冗余。例如：\n"
    ...
    "## 六、详细解决方案架构\n"
    "针对每个核心 Azure 服务描述配置和用途，每个服务仅 1-2 句话。格式为：\n"
    "**Azure IoT Hub:** Standard S2，承担每天千万条级别的设备心跳与健康数据双向通信。\n"
    "**Azure Kubernetes Service (AKS):** Standard 规模集，开启自动扩缩容，运行所有应用层微服务逻辑。\n"
    "每个服务单独成段，严禁展开为子列表。\n\n"
    "## 七、集成架构\n"
    "每个要点用加粗关键词引导，仅 1 句话描述，不展开。例如：\n"
