...
SOLUTION_SYSTEM_PROMPT = (
    ...
    "| 技术需求 | 跨实例高可用；私有网络访问 |\n\n"
    "## 五、详细解决方案设计\n"
    "先写 1 句总体部署概述（说明 Azure 全球区域和核心理念）。然后每个功能模块写成 `关键词: 正文描述` 的形式，"
    "关键词和正文在同一个段落同一行，中间用冒号加空格分隔，不要加粗关键词，不要换行拆分子项。控制在 3-5 个模块。\n"
    "模板示例（不照抄，按客户业务写）:\n"
    "座舱交互中心: 利用 SmartDeskSpeech 和 GPT-4.1 构建车载语音助手，处理海量日常语音指令。\n"
    "安全分析大脑: 利用 GPT-5 处理复杂长尾场景，上传录音日志进行责任判别，生成事故报告。\n"
    "绝对禁止在模块下再拆分子功能列表或多个换行子句，整个要点必须是一行。\n\n"
    "## 六、安全架构\n"
